...
bool CAudioDecoder::Init(const CFileItem& file, unsigned int filecache)
{
  ...
  if (CAddonDll<DllAudioDecoder, AudioDecoder, AUDIODEC_PROPS>::CreateInstance(ADDON_INSTANCE_AUDIODECODER, ID().c_str(), m_pInfo, m_pStruct, this, &m_addonInstance) != ADDON_STATUS_OK)
    return false;
  ...
  m_context = m_pStruct->Init(m_addonInstance, file.GetPath().c_str(), filecache,
                              &channels, &sampleRate,
...
int CAudioDecoder::ReadPCM(uint8_t* buffer, int size, int* actualsize)
{
  ...
  return m_pStruct->ReadPCM(m_addonInstance, m_context, buffer, size, actualsize);
}
...
bool CAudioDecoder::Seek(int64_t time)
{
  ...
  m_pStruct->Seek(m_addonInstance, m_context, time);
  return true;
}
...
void CAudioDecoder::DeInit()
{
  ...
  m_pStruct->DeInit(m_addonInstance, m_context);
}
                         ...
                         MUSIC_INFO::EmbeddedArt* art)
{
  ...
  char artist[256];
  int length;
  if (m_pStruct->ReadTag(m_addonInstance, fileName.c_str(), title, artist, &length))
  {
    tag.SetTitle(title);
...
int CAudioDecoder::GetTrackCount(const std::string& strPath)
{
  ...
  int result = m_pStruct->TrackCount(m_addonInstance, strPath.c_str());
