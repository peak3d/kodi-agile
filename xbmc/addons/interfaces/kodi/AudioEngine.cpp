...
void Interface_AudioEngine::Init(sFuncTable_Addon* funcTable)
{
  funcTable->toKodi.kodi_audioengine = (sAddonToKodiFuncTable_kodi_audioengine*)malloc(sizeof(sAddonToKodiFuncTable_kodi_audioengine));
...
AEStreamHandle* Interface_AudioEngine::AudioEngine_MakeStream(AudioEngineFormat StreamFormat, unsigned int options)
{
  ...
  format.m_dataFormat = GetKodiAudioFormat((AudioDataFormat)StreamFormat.m_dataFormat);
  format.m_sampleRate = StreamFormat.m_sampleRate;
  
  enum AEChannel channels[AE_CH_MAX];
  for (unsigned int i = 0; i < AUDIO_CH_MAX && i < AE_CH_MAX; ++i)
    channels[i] = GetKodiAudioChannel(StreamFormat.m_channels[i]);
  ...
  return CAEFactory::MakeStream(format, kodiOption);
}
...
void Interface_AudioEngine::AudioEngine_FreeStream(AEStreamHandle *handle)
{
  ...
  CAEFactory::FreeStream(static_cast<IAEStream*>(handle));
}
...
bool Interface_AudioEngine::AudioEngine_GetCurrentSinkFormat(void *kodiInstance, AudioEngineFormat *format)
{
  ...
  if (!CAEFactory::GetEngine() || !CAEFactory::GetEngine()->GetCurrentSinkFormat(sinkFormat))
  {
  ...
  {
    format->m_channels[ch] = (AudioChannel)GetAddonAudioChannel(sinkFormat.m_channelLayout[ch]);
  }

  format->m_dataFormat = (AudioDataFormat)GetAddonAudioFormat(sinkFormat.m_dataFormat);
...
const AudioDataFormat Interface_AudioEngine::AEStream_GetDataFormat(void *kodiInstance, AEStreamHandle *handle)
{
  if (!kodiInstance || !handle)
  {
    CLog::Log(LOGERROR, "Interface_AudioEngine::%s - invalid stream data", __FUNCTION__);
  ...
  return (AudioDataFormat)GetAddonAudioFormat(static_cast<IAEStream*>(handle)->GetDataFormat());
}
